  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        alert('Compte cree ! Verifiez votre email.')
      } else {
      ...
      <div className="w-full max-w-md">
        <div className="glass-card p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold mb-2">{isSignUp ? 'Rejoignez FANZONE' : 'Bienvenue sur FANZONE'}</h1>
            <p className="text-gray-400">{isSignUp ? 'Creez votre compte et commencez' : 'Connectez-vous a votre compte'}</p>
          </div>

          <div className="grid grid-cols-2 gap-3 mb-6">
            <button onClick={() => signInWithOAuth('google')} className="btn-secondary flex items-center justify-center gap-2 text-sm">Google</button>
            <button onClick={() => signInWithOAuth('github')} className="btn-secondary flex items-center justify-center gap-2 text-sm">GitHub</button>
          </div>

          <div className="relative mb-6">
            <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-white/10"></div></div>
            <div className="relative flex justify-center text-sm"><span className="px-2 bg-fanzone-card text-gray-400">ou avec email</span></div>
          </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => setFormData({ ...formData, role: 'fan' })}
                    ...
                    <div className="text-lg mb-1">Fan</div><div className="font-medium text-sm">Fan</div><div className="text-xs text-gray-400">Soutenir des createurs</div>
                  </button>
                  <button type="button" onClick={() => setFormData({ ...formData, role: 'creator' })}
                    ...
                    <div className="text-lg mb-1">Createur</div><div className="font-medium text-sm">Createur</div><div className="text-xs text-gray-400">Monetiser mon contenu</div>
                  </button>
                </div>
              </div>
            )}
            <button type="submit" disabled={isLoading} className="w-full btn-primary disabled:opacity-50">
              {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mx-auto" /> : (isSignUp ? 'Creer mon compte' : 'Se connecter')}
            </button>
          </form>

          <p className="text-center mt-6 text-sm text-gray-400">
            {isSignUp ? 'Deja un compte ?' : "Pas encore de compte ?"}{' '}
            <button onClick={() => setIsSignUp(!isSignUp)} className="text-fanzone-accent hover:underline font-medium">{isSignUp ? 'Se connecter' : "S'inscrire"}</button>
          </p>
        </div>
      </div>
    </div>
