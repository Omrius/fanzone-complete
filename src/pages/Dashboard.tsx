...
export default function Dashboard() {
  ...
  if (authLoading || isLoading) return <div className="flex items-center justify-center min-h-screen">{t('common.loading')}</div>
  if (!isAuthenticated) return <Navigate to="/auth" />
  if (user?.role !== 'creator') return <Navigate to="/" />

  const statCards = [
    { label: t('dashboard.totalEarnings'), value: `${totalEarnings.toFixed(2)}`, icon: DollarSign, color: 'text-green-400' },
    { label: t('dashboard.thisMonth'), value: `${(totalEarnings * 0.3).toFixed(2)}`, icon: TrendingUp, color: 'text-fanzone-accent' },
    { label: t('dashboard.tipsReceived'), value: transactions.filter(t => t.type === 'tip_received').length.toString(), icon: Heart, color: 'text-pink-400' },
    { label: t('dashboard.subscribers'), value: '0', icon: Users, color: 'text-blue-400' },
  ]
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold mb-4">{t('dashboard.stripePayments')}</h2>
          ...
          <SpotifyEmbed />
          <div className="glass-card p-6">
            <h2 className="text-lg font-semibold mb-4">{t('dashboard.topSupporters')}</h2>
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
